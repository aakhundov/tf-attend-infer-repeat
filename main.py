...
for i in range(2):
    models.append(
        AIRModel(
            ...
            z_pres_prior_log_odds=-10.0, z_pres_temperature=1.0, stopping_threshold=0.99,
            learning_rate=1e-4, gradient_clipping_norm=10.0,
            num_summary_images=NUM_IMAGES_TO_SAVE, train=(i == 0), reuse=(i == 1), scope="air",
            annealing_schedules={
                # "z_pres_prior_log_odds": {"init": -1.0, "min": -10.0, "factor": 1.1, "iters": 1000},
                # "z_pres_temperature": {"init": 10.0, "min": 0.1, "factor": 0.8, "iters": 1000},
                # "learning_rate": {"init": 1e-3, "min": 1e-4, "factor": 0.7, "iters": 1000}
            }
        )
    )
